...
from tensorflow.keras.layers import (
    Embedding, LayerNormalization, UpSampling2D)

# Multi-Head Attention Layer. #
class MultiHeadAttention(tf.keras.layers.Layer):
    def __init__(
        self, d_model, n_heads, 
        ker_sz, name="multi_head_attn_layer"):
    ...
    def call(self, v, k, q):
        ...
        q_pad = tf.concat([zero_pad, q], axis=2)
        k_pad = tf.concat([zero_pad, k], axis=2)
        v_pad = tf.concat([zero_pad, v], axis=2)
...
class DecoderLayer(tf.keras.layers.Layer):
    def __init__(
        ...
        self.attn_self = MultiHeadAttention(
            d_model, n_heads, ker_sz, name=name)
...
class Decoder(tf.keras.layers.Layer):
    def __init__(
        ...
        max_seq_length, ker_sz, rate1=0.1, rate2=0.1):
        ...
        for n_layer in range(n_layers):
            tmp_dec_layers.append(DecoderLayer(
                ...
                self.ker_sz, rate1=rate1, rate2=rate2, 
...
class GPTUpsample(tf.keras.Model):
    def __init__(
        ...
        self.gpt_model = Decoder(
            ...
            max_seq_length, ker_sz, rate1=rate1, rate2=rate2)
